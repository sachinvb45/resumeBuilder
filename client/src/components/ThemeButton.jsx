...
import { useApplicationTheme } from '../context/useApplicationTheme';
...
const MaterialUISwitch = styled(Switch)(({ theme }) => ({
  ...
  '& .MuiSwitch-switchBase': {
    margin: 1,
    padding: 0,
    transform: 'translateX(4px)', // Adjusted position
    '&.Mui-checked': {
      color: '#fff',
  ...
  '& .MuiSwitch-thumb': {
    ...
    width: 24, // Adjusted thumb width
    height: 24, // Adjusted thumb height
    '&::before': {
      content: "''",
      position: 'absolute',
      width: '100%',
...
export default function CustomizedSwitches() {
  ...
  return (
    <FormGroup>
      <FormControlLabel
        label="Theme"
        labelPlacement="start" // Label on the left side
        control={
          <MaterialUISwitch
            sx={{ m: 1 }} // Remove extra margin
            checked={theme === "dark"}
        ...
        sx={{ 
          '& .MuiFormControlLabel-label': {
            marginLeft: '-18px', // Adjust margin to reduce extra space
          }
        }}
      />
    </FormGroup>
